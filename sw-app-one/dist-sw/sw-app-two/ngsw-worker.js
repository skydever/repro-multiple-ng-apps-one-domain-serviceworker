(function () {
  ...
  class CacheDatabase {
      ...
      'delete'(name) {
          ...
          return this.scope.caches.delete(`ngsw:sw-app-two:db:${name}`);
      }
      list() {
          return this.scope.caches.keys().then(keys => keys.filter(key => key.startsWith('ngsw:sw-app-two:db:')));
      }
      open(name) {
          if (!this.tables.has(name)) {
              const table = this.scope.caches.open(`ngsw:sw-app-two:db:${name}`)
  ...
  class AppVersion {
      constructor(scope, adapter, database, idle, manifest, manifestHash) {
          ...
          this.assetGroups = (manifest.assetGroups || []).map(config => {
              ...
              // group.
              const prefix = `ngsw:sw-app-two:${this.manifestHash}:assets`;
          ...
          this.dataGroups = (manifest.dataGroups || [])
              .map(config => new DataGroup(this.scope, this.adapter, config, this.database, `ngsw:sw-app-two:${config.version}:data`));
      }
  ...
  class Driver {
      ...
      async deleteAllCaches() {
          await (await this.scope.caches.keys())
              .filter(key => key.startsWith('ngsw:sw-app-two:'))
